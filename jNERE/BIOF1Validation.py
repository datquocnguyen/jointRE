...
def compute_NER_f1(label_pred, label_correct, correctBIOErrors='No', encodingScheme='BIO'):
    ...
    prec = compute_NER_precision(label_pred, label_correct)
    rec = compute_NER_precision(label_correct, label_pred)
...
def compute_Rel_f1(label_pred, label_correct):
    prec = compute_Rel_precision(label_pred, label_correct)
    rec = compute_Rel_precision(label_correct, label_pred)
...
def compute_Rel_precision(predSentences, correctSentences):
    correctCount = 0
    count = 0
    for index in predSentences:
        for key in predSentences[index]:
            count += 1
            if key in correctSentences[index]:
                if correctSentences[index][key] == predSentences[index][key]:
                    correctCount += 1
    precision = 0
    if count > 0:
        precision = float(correctCount) / count
...
def compute_f1_bk(predictions, correct, idx2Label, correctBIOErrors='No', encodingScheme='BIO'):
    label_pred = []
    ...
    return compute_NER_f1(label_pred, label_correct, correctBIOErrors, encodingScheme)
...
def convertIOBEStoBIO(dataset):
    ...
    for sentence in dataset:
        for pos in range(len(sentence)):
            firstChar = sentence[pos][0]
            if firstChar == 'S' or firstChar == 'U':
                sentence[pos] = 'B'+sentence[pos][1:]
            elif firstChar == 'E' or firstChar == 'L':
...
def compute_NER_precision(guessed_sentences, correct_sentences):
    assert(len(guessed_sentences) == len(correct_sentences))
    correctCount = 0
    count = 0
    ...
    for sentenceIdx in range(len(guessed_sentences)):
        ...
        while idx < len(guessed):
            if guessed[idx][0] == 'B' and guessed[idx] != "B-Other": #A new chunk starts
                count += 1
...
def compute_NER_precision_label(guessed_sentences, correct_sentences, label):
    ...
    for sentenceIdx in range(len(guessed_sentences)):
        ...
        idx = 0
        while idx < len(guessed):
            if guessed[idx] == label: #A new chunk starts
                count += 1
...
def compute_Rel_f1_macro(label_pred, label_correct, nerids = None):
    rels = ['Work_For', 'OrgBased_In', 'Live_In', 'Kill', 'Located_In']

    f1_macro = 0.0
    for rel in rels:
        prec = compute_Rel_precision_macro(label_pred, label_correct, rel, nerids)
        rec = compute_Rel_precision_macro(label_correct, label_pred, rel, nerids)
    ...
    return f1_macro / len(rels)

def compute_Rel_precision_macro(predSentences, correctSentences, rel, nerids):
    correctCount = 0
    count = 0
    for index in predSentences:
        for key in predSentences[index]:
            if predSentences[index][key] == rel:
                count += 1
                if nerids is not None:
                    (id1, id2) = key
                    if id1 not in nerids[index] or id2 not in nerids[index]:
                        continue
                if key in correctSentences[index]:
                    if correctSentences[index][key] == predSentences[index][key]:
                        correctCount += 1
...
def getCorrectNERids(nerpred, test_id2nerBILOU):

    neridslist = {}
    for sentenceIdx in nerpred:
        nerids = []
        guessed = [convert2BIO(tag) for tag in nerpred[sentenceIdx]]
        correct = [convert2BIO(tag) for tag in test_id2nerBILOU[sentenceIdx].split()]
        #print("----")
        #print(guessed)
        #print(correct)
        assert len(guessed) == len(correct)
