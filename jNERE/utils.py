...
import gzip
import pickle
from  BIOF1Validation import compute_Rel_f1_macro, getCorrectNERids
import numpy as np
...
if __name__ == '__main__':
    ...
    words, w2i, c2i, nertags, postags = vocabNER(train_id2sent, train_id2pos, train_id2nerBILOU)
    data = readData(test_id2sent, test_id2pos, c2i)


    (nerpred, relpred) = pickle.load(open( "outputs/output.predtest_ep9", "rb" ))




    a = getCorrectNERids(nerpred, test_id2nerBILOU)
    f1_rel = compute_Rel_f1_macro(relpred, test_id2arg2rel, a)
#    print "RC F1 on test set: {:.3f}".format(f1_rel)
