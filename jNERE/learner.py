...
from dynet import *
import dynet
from utils import load_embeddings_file, readData, orthonormal_initializer
...
class jNeRE:
    def __init__(self, vocab, ner, rels, w2i, c2i, options):
        ...
        self.ner_builders = [VanillaLSTMBuilder(1, self.wdims + self.cdims * 2, self.ldims, self.model),
        ...
        if self.bibiFlag:
            self.builders = [
                VanillaLSTMBuilder(1, self.wdims + self.cdims * 2 + self.nerdims, self.ldims, self.model),
                VanillaLSTMBuilder(1, self.wdims + self.cdims * 2 + self.nerdims, self.ldims, self.model)]
            self.bbuilders = [VanillaLSTMBuilder(1, self.ldims * 2, self.ldims, self.model),
                              VanillaLSTMBuilder(1, self.ldims * 2, self.ldims, self.model)]
        elif self.layers > 0:
            self.builders = [
                VanillaLSTMBuilder(self.layers, self.wdims + self.cdims * 2 + self.nerdims, self.ldims,
                                   self.model),
                VanillaLSTMBuilder(self.layers, self.wdims + self.cdims * 2 + self.nerdims, self.ldims,
                                   self.model)]
        else:
            self.builders = [
                SimpleRNNBuilder(1, self.wdims + self.cdims * 2 + self.nerdims, self.ldims, self.model),
                SimpleRNNBuilder(1, self.wdims + self.cdims * 2 + self.nerdims, self.ldims, self.model)]
        ...
        if isTrain:
            ...
            for iSentence, sentenceId in enumerate(shuffledData):
                ...
                for entry in sentence:
                    ...
                    entry.vec = dynet.dropout(concatenate(filter(None, [wordvec, last_state, rev_last_state])),
                                              0.33)
