...
def compute_Rel_f1_macro(label_pred, label_correct):
    rels = ['Work_For', 'OrgBased_In', 'Live_In', 'Kill', 'Located_In']

    f1_macro = 0.0
    for rel in rels:
        prec = compute_Rel_precision_macro(label_pred, label_correct, rel)
        rec = compute_Rel_precision_macro(label_correct, label_pred, rel)
...
def compute_Rel_precision_macro(predSentences, correctSentences, rel):
    correctCount = 0
    count = 0
    for index in predSentences:
        for key in predSentences[index]:
