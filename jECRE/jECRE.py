...
from utils import readCoNLL2004_prepared_corpus, vocabNER, readData, getRelVocab
from BIOF1Validation import compute_NER_f1_macro, compute_Rel_f1_macro
import logging
...
if __name__ == '__main__':
        ...
        for index in train_id2pos:
            tags = [tag[:1] for tag in train_id2nerBILOU[index].split()]
            train_id2pos[index] = " ".join(tags).strip()

        for index in test_id2pos:
            tags = [tag[:1] for tag in test_id2nerBILOU[index].split()]
            test_id2pos[index] = " ".join(tags).strip()

        words, w2i, c2i, nertags, postagCount = vocabNER(train_id2sent, train_id2pos, train_id2ner)
        ...
        parser = learner.jEcRE(words, nertags, postagCount, rels, w2i, c2i, options)

        for epoch in xrange(options.epochs):
            print '\n-----------------\nStarting epoch', epoch + 1

            #parser.Train(train_data, train_id2nerBILOU, id2arg2rel, classweights)
            parser.Train(train_data, train_id2ner, id2arg2rel)
            ...
            print "EC macro-averaged F1 on dev: {:.3f}".format(f1)

            f1_rel = compute_Rel_f1_macro(relsDev, dev_id2arg2rel)
            print "RC macro-averaged F1 on dev: {:.3f}".format(f1_rel)

            score = (f1 + f1_rel) / 2
            print "(EC-F1 + RC-F1)/2 on dev:", score
            if score >= highestScore:
                #parser.Save(os.path.join(options.output, os.path.basename(options.model)))
                highestScore = score
                eId = epoch + 1

            print "Highest (EC-F1 + RC-F1)/2 on dev: %.3f at epoch %d" % (highestScore, eId)
            ...
            print "NER macro-averaged F1 on test: {:.3f}".format(f1)

            f1_rel = compute_Rel_f1_macro(relsDev, test_id2arg2rel)
